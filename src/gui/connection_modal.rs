...
impl App {
        ...
        if let Some(con) = &mut self.gui_state.connection_modal_state {
            modal.show(ctx, |ui| {
                Grid::new("data").min_col_width(80.0).show(ui, |ui| {
                    let r1 = modal_label(ui, "Hostname", &mut con.hostname, false).changed();
                    ui.end_row();
                    let r2 = modal_label(ui, "Port", &mut con.port, false).changed();
                    ui.end_row();

                    let r3 = modal_label(ui, "Virtual Host", &mut con.vhost, false).changed();
                    ui.end_row();

                    let r4 = modal_label(ui, "Username", &mut con.username, false).changed();
                    ui.end_row();

                    let r5 = modal_label(ui, "Password", &mut con.password, true).changed();
                    ui.end_row();

                    ui.label("");
                    let r6 = ui
                        .checkbox(&mut con.tls, RichText::new("Enable TLS").size(16.0))
                        .changed();
                    ui.end_row();
            ...
            match modal_result {
                ModalResult::Ok => {
                    ...
                    self.gui_state.connection_state.validate();
    ...
    fn close_connection_modal(&mut self) {
        self.gui_state.connection_modal_state = None;
    }
}
